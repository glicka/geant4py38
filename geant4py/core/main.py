...
from geant4py.geo.volume import get_all_children as volGetAllChild
from geant4py.geo.gdml import get_all_children as gdmlGetAllChild
    ...
    def __init__(self, world, source, check_overlaps=False, track_secondaries=False,
        ...
        if self.check_overlaps:
            if self.world.grade() == 'GDML':
                vols = gdmlGetAllChild(self.world)
            elif self.world.grade() == 'vol':
                vols = volGetAllChild(self.world)
            for vol in vols:
                vol.check_overlaps()
        # Start sim
        self.execute()
    ...
    def execute(self):
        ...
        if self.multiprocess:
            ...
            if not isinstance(self.source, list):
                ...
                if sum(particles) != self.source.num_particles:
                    particles.append(self.source.num_particles - sum(particles))
            ...
            start_time = time.time()
            numLoops = 1
            iterableInt = int(numLoops*ncpus)
            iterable = int(len(self.source)*1./(iterableInt))
            print('iterables = ', iterable)
            print('ncpus = ', ncpus)
            '''
            Loop over number of iterables and run simulation. If numLoops = 1,
            there is just 1 iteration through the loop. This saves on memory as
            tracking all data is memory intensive.
            '''
