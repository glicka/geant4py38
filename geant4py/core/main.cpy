...
@ray.remote
def BeamOn(self,source):#,detector=None):
...
class Run():
    ...
    def execute(self):
        ...
        if self.multiprocess:
            ...
            """
            start_time = time.time()
            #actors = [MyActor.remote() for _ in range(ncpus)]
            pools = Pool(ncpus)#ActorPool(actors)
            #pool.map(lambda a, v: a.BeamOn.remote(v),self.source)
            #pool.submit(lambda a, v: a.BeamOn.remote(v),self.source)
            pools.map(BeamOn,self.source)
